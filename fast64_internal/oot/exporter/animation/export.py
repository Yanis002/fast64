import bpy
from bpy.types import Bone, Object
...
def ootGetAnimBoneRot(bone: Bone, poseBone: Bone, isRoot: bool):
    ...
    origTranslationMatrix = (
        bone.parent.matrix_local.inverted() if bone.parent is not None else Matrix.Identity(4)
    ) @ bone.matrix_local

    origTranslation = origTranslationMatrix.decompose()[0]
    inverseTranslationMatrix = Matrix.Translation(origTranslation).inverted()

    animMatrix = (
        inverseTranslationMatrix
        @ (poseBone.parent.matrix.inverted() if poseBone.parent is not None else Matrix.Identity(4))
        @ poseBone.matrix
    )
    ...
    if isRoot:
        ...
        zUpToYUp = Quaternion((1, 0, 0), radians(-90.0))
    ...
    if not isRoot and (max(finalTranslation) >= 1.0 or min(finalTranslation) <= -1.0):
        raise RuntimeError(
            "Animation contains non-root bones with animated translation. "
            + "OoT SkelAnime only supports animated translation on the root bone."
        )
    ...
    for frame in range(frame_start, frame_start + frame_count):
        ...
        for boneIndex in range(len(animBones)):
            ...
            saveQuaternionFrame(
                ...
                ootGetAnimBoneRot(currentBone, currentPoseBone, boneIndex == 0),
            )
...
def ootExportAnimationCommon(armatureObj: Object, convertTransformMatrix: Matrix, skeletonName: str):
